...
CONTRAST = 1.15
BRIGHTNESS = 1.0
GAMMA = 0.72          # <1 darkens mids so a bright face still gets characters
SHARPEN = False
WHITE_FLOOR = 0.95    # luminance above this is forced to blank (space)
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">arthur@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'arthur@github:~$ whoami <tspan fill="{INK}">Arthur Hawreliuk</tspan></text>')
parts.append(f'<rect x="{PAD+231}" y="{status_y-12:.1f}" width="8" height="14" fill="{INK}">'
